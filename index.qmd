...
## Project Overview ##
This project is about the dynamic concepts of economics, to chart the course of global GDP growth rates from the years 1999 to 2022. Because of RStudio, it is possible to  transform raw data into a narrative that reflects past economic trends, as well as help in forecasting future possibilities. The graph below is a visualizations which brings clarity to economics that may seem complex, making them accessible to anyone. The countries with the highest change in GDP over 23 years are China and the United States.
...
## Bar Graphs ##
A bar graph consists of "boxes" that are designed to display the value of a country. Below are two bar graphs that show the values of five different countries at different points of time. An advantage of this is that it is easier to read than the line plot. However, a disadvantage compared to the line plot is that we can only see the values of one specific year at a time, rather than all the years in our dataset.
```{r}
library(tidyverse)
library(ggrepel)
...
## Chloropleth ##
Our final graphic is a chloropleth. A chloropleth is a thematic map that displays data on a map. In the examples below, we can see a different color represents a different range of GDP values. Here, we can view all the countries. The only disadvantage is that, similar to the bar graphs, we can only see the data for one specific year, and not all the years.
```{r}
library(ggplot2)
library(dplyr)
library(readr)
